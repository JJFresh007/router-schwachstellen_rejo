...
\setcounter{tocdepth}{3}                   % Setzt die Tiefe des Inhaltsverzeichnisses auf 3 Ebenen: Kapitel (chapter), Abschnitt (section), Unterabschnitt (subsection)
\usepackage{tabularx}                      % Ermöglicht erweiterte Tabellenfunktionen, insbesondere Tabellen mit automatisch anpassbarer Spaltenbreite

\usepackage[ngerman]{babel}                % Stellt Spracheinstellungen auf neues Deutsch um, beeinflusst z.B. Datumformat und Silbentrennung
\usepackage[german]{translator}            % Wird für die Übersetzung von Strings in Paketen verwendet, die nicht direkt von babel unterstützt werden

\usepackage[T1]{fontenc}                   % Wählt eine Schriftkodierung, die eine bessere Darstellung von Umlauten und anderen europäischen Zeichen ermöglicht
\usepackage{microtype}                     % Verbessert den Textsatz durch Mikrotypografie

\usepackage{fixme}                         % Ermöglicht das Hinzufügen von Kommentaren und Notizen im Dokument, die leicht auffindbar sind
\fxsetup{status=draft}                     % Legt den Status des Dokuments fest; 'draft' bedeutet, dass Notizen sichtbar sind

\usepackage{minted}                        % Ermöglicht das Hervorheben von Quellcode mit Syntaxfarben, basierend auf Pygments

\setminted{
  breaklines=true,                         % Erlaubt automatischen Zeilenumbruch im Quellcode
  breakanywhere=true,                      % Erlaubt Zeilenumbrüche an beliebiger Stelle (nützlich für sehr lange Zeilen)
  breaksymbolleft=,                        % Definiert das Symbol für einen Zeilenumbruch am Zeilenanfang
  breaksymbolright=,                       % Definiert das Symbol für einen Zeilenumbruch am Zeilenende
  breaksymbolsepleft=,                     % Definiert das Trennsymbol für Umbrüche am Zeilenanfang
  breaksymbolsepright=,                    % Definiert das Trennsymbol für Umbrüche am Zeilenende
  breaksymbolindentleft=,                  % Definiert den Einzug für das Umbruchsymbol am Zeilenanfang
  breaksymbolindentright=,                 % Definiert den Einzug für das Umbruchsymbol am Zeilenende
  breakanywheresymbolpre=,                 % Definiert das Symbol vor einem Zeilenumbruch bei 'breakanywhere'
  breakanywheresymbolpost=,                % Definiert das Symbol nach einem Zeilenumbruch bei 'breakanywhere'
  linenos=true,                            % Fügt Zeilennummern zum Quellcode hininzu
  xleftmargin=20pt,                        % Definiert den linken Rand des Quellcode-Blocks
  frame=lines                              % Fügt eine Umrahmung aus Linien um den Quellcode-Block hinzu
}
...
% Auskommentiert aufgrund von 0x01
...
\graphicspath{{img/}}                      % Bilder können aus dem Verzeichnis img ohne Pfadangabe
                                           % eingebunden werden
...
\usepackage[backend = biber,               % Standardmäßig ab TeXLive 2016
            sorting = none,                % Sortieren nach erster Nennung
            style   = ieee]{biblatex}      % Bibliographie im IEEE-Stil
\addbibresource{Literatur_rebe.bib}
...
\cofoot*{}
\rofoot*{\pagemark}
% Dieses Paket erzwingt LuaLatex. Damit muss der Font-Cache berechnet werden. 
% Dies führt zu einer viel langsameren Kompilierung des Projekts.   ID: 0x01
